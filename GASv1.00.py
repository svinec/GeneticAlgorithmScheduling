...
class GAS():
	def __init__( self, _parameters ):
		self.operationDurations = dict( _parameters[ "operationDurations" ] )
		...
		self.operationMaxTime = sum( self.operationDurations.values() )
		for op2 in self.operationRelations:
			for op1 in self.operationRelations[ op2 ]:
	...
	def scorePopulation( self ):
		for p in self.population:
			p[ "score" ] = 0
		#self.scorePopulation_totalDuration()
		self.scorePopulation_operationRelations()
		self.scorePopulation_resourceSuccession()
		return True
		
	def scorePopulation_totalDuration( self ):
		for p in self.population:
			max_duration = 0
			for i in range( self.operationCount ):
				duration = p[ "start_times" ][ i ] + self.operationDurations[ i ]
				if duration > max_duration: max_duration = duration
			p[ "score" ] -= max_duration
		return True
			
	def scorePopulation_operationRelations( self ):
		for p in self.population:
			for op2 in self.operationRelations:
				start2 = p[ "start_times" ][ op2 ]
				end2 = p[ "start_times" ][ op2 ] + self.operationDurations[ op2 ]
				
				for op1 in self.operationRelations[ op2 ]:
					start1 = p[ "start_times" ][ op1 ]
					end1 = p[ "start_times" ][ op1 ] + self.operationDurations[ op1 ]
	...
	def scorePopulation_resourceSuccession( self ):
		for p in self.population:
			sorted_operations = []
			for i in range( self.operationCount ):
			...
			for i in range( 1, self.operationCount ):
				...
				if r1 == r2:
					if p[ "start_times" ][ op2 ] < p[ "start_times" ][ op1 ] + self.operationDurations[ op1 ]:
	...
	def printBestNormalized( self ):
		...
		print( "normalized: {}, start_times: {}, resources: {}".format( self.population[ 0 ][ "score" ], start_times, self.population[ 0 ][ "resources" ] ) )
